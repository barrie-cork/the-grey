...
class PerformanceAnalyticsService(ServiceLoggerMixin):
    ...
    def calculate_search_performance_metrics(self, session_id: str) -> Dict[str, Any]:
        ...
        metrics = {
            'search_efficiency': {
                ...
                'deduplication_rate': round((total_raw_results - unique_processed) / max(total_raw_results, 1) * 100, 1),
                'processing_rate': round(unique_processed / max(total_raw_results, 1) * 100, 1)
            },
            'relevance_metrics': {
                'precision': round(include_count / max(include_count + exclude_count, 1) * 100, 1),
                'inclusion_rate': round(include_count / max(unique_processed, 1) * 100, 1),
                'number_needed_to_read': round(unique_processed / max(include_count, 1), 1)
            },
            'cost_effectiveness': {
                'total_cost': sum(e.estimated_cost for e in executions),
                'cost_per_result': round(
                    float(sum(e.estimated_cost for e in executions)) / max(unique_processed, 1), 4
                ),
                'cost_per_included_study': round(
                    float(sum(e.estimated_cost for e in executions)) / max(include_count, 1), 4
                ) if include_count > 0 else 0
            },
            'time_efficiency': {
                'avg_execution_time': round(
                    sum(e.duration_seconds or 0 for e in executions) / max(successful_executions, 1), 1
                ),
    ...
    def calculate_engine_comparison_metrics(self, session_id: str) -> Dict[str, Any]:
        ...
        for engine, metrics in engine_metrics.items():
            ...
            metrics['success_rate'] = round(
                successful_exec / max(total_exec, 1) * 100, 1
            )
            metrics['avg_results_per_execution'] = round(
                metrics['total_results'] / max(successful_exec, 1), 1
            )
            metrics['avg_cost_per_execution'] = round(
                metrics['total_cost'] / max(successful_exec, 1), 4
            )
            metrics['avg_time_per_execution'] = round(
                metrics['total_time'] / max(successful_exec, 1), 1
            )
            
            # Cost effectiveness
            metrics['cost_per_result'] = round(
                metrics['total_cost'] / max(metrics['total_results'], 1), 4
            )
    ...
    def calculate_temporal_performance(self, session_id: str) -> Dict[str, Any]:
        ...
        for date_key, metrics in daily_metrics.items():
            if metrics['times']:
                metrics['avg_time'] = round(sum(metrics['times']) / len(metrics['times']), 1)
            
            metrics['success_rate'] = round(
                metrics['successful'] / max(metrics['executions'], 1) * 100, 1
            )
    ...
    def generate_efficiency_recommendations(self, session_id: str) -> Dict[str, Any]:
        ...
        if engine_comparison:
            ...
            if best_engine[1]['success_rate'] - worst_engine[1]['success_rate'] > 20:
                recommendations['recommendations'].append({
        ...
        recommendations['priority_actions'] = high_priority[:3]  # Top 3 high priority items
