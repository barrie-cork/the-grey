...
class PerformanceAnalyticsService(ServiceLoggerMixin):
    ...
    def calculate_search_performance_metrics(self, session_id: str) -> Dict[str, Any]:
        ...
        exclude_count = SimpleReviewDecision.objects.filter(
            result__session_id=session_id,
            tag__name=PerformanceConstants.EXCLUDE_TAG
        ).count()
        ...
        metrics = {
            'search_efficiency': {
                'success_rate': round(
                    successful_executions / max(total_executions, PerformanceConstants.MIN_DIVISOR) * 
                    PerformanceConstants.PERCENTAGE_MULTIPLIER, 
                    PerformanceConstants.DECIMAL_PLACES['percentage']
                ),
                'deduplication_rate': round(
                    (total_raw_results - unique_processed) / max(total_raw_results, PerformanceConstants.MIN_DIVISOR) * 
                    PerformanceConstants.PERCENTAGE_MULTIPLIER,
                    PerformanceConstants.DECIMAL_PLACES['percentage']
                ),
                'processing_rate': round(
                    unique_processed / max(total_raw_results, PerformanceConstants.MIN_DIVISOR) * 
                    PerformanceConstants.PERCENTAGE_MULTIPLIER,
                    PerformanceConstants.DECIMAL_PLACES['percentage']
                )
            },
            'relevance_metrics': {
                'precision': round(
                    include_count / max(include_count + exclude_count, PerformanceConstants.MIN_DIVISOR) * 
                    PerformanceConstants.PERCENTAGE_MULTIPLIER,
                    PerformanceConstants.DECIMAL_PLACES['percentage']
                ),
                'inclusion_rate': round(
                    include_count / max(unique_processed, PerformanceConstants.MIN_DIVISOR) * 
                    PerformanceConstants.PERCENTAGE_MULTIPLIER,
                    PerformanceConstants.DECIMAL_PLACES['percentage']
                ),
                'number_needed_to_read': round(
                    unique_processed / max(include_count, PerformanceConstants.MIN_DIVISOR),
            ...
            'cost_effectiveness': {
                ...
                'cost_per_result': round(
                    float(sum(e.estimated_cost for e in executions)) / max(unique_processed, PerformanceConstants.MIN_DIVISOR),
                    PerformanceConstants.DECIMAL_PLACES['cost']
                ),
                'cost_per_included_study': round(
                    float(sum(e.estimated_cost for e in executions)) / max(include_count, PerformanceConstants.MIN_DIVISOR),
                    PerformanceConstants.DECIMAL_PLACES['cost']
                ) if include_count > 0 else 0
            },
            'time_efficiency': {
                'avg_execution_time': round(
                    sum(e.duration_seconds or 0 for e in executions) / max(successful_executions, PerformanceConstants.MIN_DIVISOR),
                    PerformanceConstants.DECIMAL_PLACES['time']
                ),
                'total_execution_time': sum(e.duration_seconds or 0 for e in executions)
            }
        }
    ...
    def calculate_engine_comparison_metrics(self, session_id: str) -> Dict[str, Any]:
        ...
        for engine, metrics in engine_metrics.items():
            ...
            metrics['success_rate'] = round(
                successful_exec / max(total_exec, PerformanceConstants.MIN_DIVISOR) * 
                PerformanceConstants.PERCENTAGE_MULTIPLIER, 
                PerformanceConstants.DECIMAL_PLACES['percentage']
            )
            metrics['avg_results_per_execution'] = round(
                metrics['total_results'] / max(successful_exec, PerformanceConstants.MIN_DIVISOR),
                PerformanceConstants.DECIMAL_PLACES['ratio']
            )
            metrics['avg_cost_per_execution'] = round(
                metrics['total_cost'] / max(successful_exec, PerformanceConstants.MIN_DIVISOR),
                PerformanceConstants.DECIMAL_PLACES['cost']
            )
            metrics['avg_time_per_execution'] = round(
                metrics['total_time'] / max(successful_exec, PerformanceConstants.MIN_DIVISOR),
                PerformanceConstants.DECIMAL_PLACES['time']
            )
            
            # Cost effectiveness
            metrics['cost_per_result'] = round(
                metrics['total_cost'] / max(metrics['total_results'], PerformanceConstants.MIN_DIVISOR),
                PerformanceConstants.DECIMAL_PLACES['cost']
            )
    ...
    def calculate_temporal_performance(self, session_id: str) -> Dict[str, Any]:
        ...
        for date_key, metrics in daily_metrics.items():
            ...
            metrics['success_rate'] = round(
                metrics['successful'] / max(metrics['executions'], PerformanceConstants.MIN_DIVISOR) * 
                PerformanceConstants.PERCENTAGE_MULTIPLIER, 
                PerformanceConstants.DECIMAL_PLACES['percentage']
            )
    ...
    def generate_efficiency_recommendations(self, session_id: str) -> Dict[str, Any]:
        ...
        if success_rate < PerformanceConstants.SUCCESS_RATE_THRESHOLD:
            recommendations['recommendations'].append({
                'category': PerformanceConstants.RECOMMENDATION_CATEGORIES[0],  # execution_reliability
        ...
        if precision < PerformanceConstants.PRECISION_THRESHOLD:
            recommendations['recommendations'].append({
                'category': PerformanceConstants.RECOMMENDATION_CATEGORIES[1],  # search_precision
                'priority': PerformanceConstants.PRIORITY_HIGH,
        ...
        cost_per_study = performance_metrics['cost_effectiveness']['cost_per_included_study']
        if cost_per_study > PerformanceConstants.COST_PER_STUDY_THRESHOLD:
            recommendations['recommendations'].append({
                'category': PerformanceConstants.RECOMMENDATION_CATEGORIES[2],  # cost_optimization
        ...
        if engine_comparison:
            ...
            if best_engine[1]['success_rate'] - worst_engine[1]['success_rate'] > PerformanceConstants.ENGINE_PERFORMANCE_DIFF_THRESHOLD:
                recommendations['recommendations'].append({
                    'category': PerformanceConstants.RECOMMENDATION_CATEGORIES[3],  # engine_optimization
                    'priority': PerformanceConstants.PRIORITY_MEDIUM,
        ...
        # Priority actions
        high_priority = [r for r in recommendations['recommendations'] if r['priority'] == PerformanceConstants.PRIORITY_HIGH]
        recommendations['priority_actions'] = high_priority[:PerformanceConstants.MAX_PRIORITY_ACTIONS]
