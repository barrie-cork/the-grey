...
class PrismaReportingService(ServiceLoggerMixin):
    ...
    def generate_prisma_flow_data(self, session_id: str) -> Dict[str, Any]:
        ...
        flow_data = {
            ...
            'included': {
                ...
                'studies_in_synthesis': review_stats['included_results']  # Assuming all included are in synthesis
            },
            'metadata': {
                'search_date': session_data['created_at'][:10],  # Extract date part
                'last_updated': session_data['created_at'][:10],  # Use created_at for now
        ...
        for execution in executions_data:
            ...
            flow_data['identification']['records_by_source'][engine] += execution.get('results_count', 0)
    ...
    def calculate_review_period(self, session) -> Dict[str, Any]:
        ...
        period_info = {
            ...
            'end_date': session.completed_at.date() if session.completed_at else timezone.now().date(),
            'duration_days': 0,
            'phases': []
        }
        ...
        checklist_items = [
            {
                ...
                'completed': bool(session.title and 'systematic' in session.title.lower()),
            ...
            },
            {
                ...
                'completed': bool(session.description),
                'evidence': 'Objectives in description' if session.description else None
            }
