...
from apps.reporting.constants import PerformanceConstants, StudyAnalysisConstants
...
class StudyAnalysisService(ServiceLoggerMixin):
        ...
        for study in included_studies:
            ...
            if year:
                characteristics["summary_statistics"]["publication_years"][year] = (
                    characteristics["summary_statistics"]["publication_years"].get(
                        year, 0
                    )
                    + 1
                )
            ...
            characteristics["summary_statistics"]["document_types"][doc_type] = (
                characteristics["summary_statistics"]["document_types"].get(doc_type, 0)
                + 1
            )
            ...
            if study.source_organization:
                ...
                characteristics["summary_statistics"]["source_organizations"][org] = (
                    characteristics["summary_statistics"]["source_organizations"].get(
                        org, 0
                    )
                    + 1
                )
    ...
    def analyze_study_quality_distribution(self, session_id: str) -> Dict[str, Any]:
        ...
        quality_analysis = {
            ...
            "publication_recency": {"last_5_years": 0, "last_10_years": 0, "older": 0},
        }
        ...
        for study in included_studies:
            ...
            if (
                ...
                and study.publication_year
                >= current_year - StudyAnalysisConstants.RECENCY_PERIODS["recent"]
            ):
            ...
            if study.publication_year:
                if (
                    ...
                    study.publication_year
                    >= current_year
                    - StudyAnalysisConstants.RECENCY_PERIODS["fairly_recent"]
                ):
    ...
    def generate_study_geographical_distribution(
        self, session_id: str
    ) -> Dict[str, Any]:
        """
        ...
        for study in included_studies:
            ...
            if not country and study.url:
                ...
                for (
                    domain,
                    mapped_country,
                ) in StudyAnalysisConstants.DOMAIN_TO_COUNTRY.items():
