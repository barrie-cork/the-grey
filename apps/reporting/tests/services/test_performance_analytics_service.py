...
class TestPerformanceAnalyticsService(TestCase):
    ...
    def setUp(self):
        ...
        self.query1 = SearchQuery.objects.create(
            session=self.session, query_string="test query 1", search_type="standard"
        )

        self.query2 = SearchQuery.objects.create(
            session=self.session, query_string="test query 2", search_type="academic"
        )
    ...
    def test_calculate_session_performance_metrics(self):
        ...
        metrics = self.service.calculate_session_performance_metrics(
            str(self.session.id)
        )
    ...
    def test_calculate_time_series_metrics(self):
        ...
        time_series = self.service.calculate_time_series_metrics(
            str(self.session.id), interval="daily"
        )
    ...
    def test_compare_search_engines_performance(self):
        ...
        comparison = self.service.compare_search_engines_performance(
            str(self.session.id)
        )
    ...
    def test_identify_performance_bottlenecks(self):
        ...
        bottlenecks = self.service.identify_performance_bottlenecks(
            str(self.session.id)
        )
    ...
    def test_calculate_cost_efficiency_metrics(self):
        ...
        efficiency = self.service.calculate_cost_efficiency_metrics(
            str(self.session.id)
        )
    ...
    def test_generate_optimization_suggestions(self):
        ...
        suggestions = self.service.generate_optimization_suggestions(
            str(self.session.id)
        )
    ...
    def test_logging_in_performance_calculations(self):
        ...
        with self.assertLogs(
            "apps.reporting.services.performance_analytics_service", level="INFO"
        ) as cm:
            self.service.calculate_session_performance_metrics(str(self.session.id))

        self.assertTrue(
            any("Calculating performance metrics" in msg for msg in cm.output)
        )
        self.assertTrue(
            any("Performance metrics calculated" in msg for msg in cm.output)
        )
