...
class TestSearchStrategyReportingService(TestCase):
    ...
    def setUp(self):
        ...
        self.population = Population.objects.create(
            ...
            time_period=f"{datetime.now().year-5}-{datetime.now().year+1}",
        )
    ...
    def test_generate_search_strategy_report(self):
        ...
        self.assertEqual(
            pic["population"]["description"], "Healthcare providers and patients"
        )
    ...
    def test_generate_reproducibility_checklist(self):
        ...
        checklist = self.service.generate_reproducibility_checklist(
            str(self.session.id)
        )
    ...
    def test_generate_search_rationale_documentation(self):
        ...
        rationale = self.service.generate_search_rationale_documentation(
            str(self.session.id)
        )
    ...
    def test_validate_search_strategy_completeness(self):
        ...
        validation = self.service.validate_search_strategy_completeness(
            str(self.session.id)
        )
    ...
    def test_logging_in_report_generation(self):
        ...
        with self.assertLogs(
            "apps.reporting.services.search_strategy_reporting_service", level="INFO"
        ) as cm:
            self.service.generate_search_strategy_report(str(self.session.id))

        self.assertTrue(
            any("Generating search strategy report" in msg for msg in cm.output)
        )
        self.assertTrue(
            any("Search strategy report generated" in msg for msg in cm.output)
        )
