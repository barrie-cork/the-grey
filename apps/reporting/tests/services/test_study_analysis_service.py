...
class TestStudyAnalysisService(TestCase):
        ...
        for i in range(50):
            ...
            if i < 30:
                ReviewTagAssignment.objects.create(
                    result=result, tag=self.tag_include, assigned_by=self.user
                )

                # Add quality tags
                if i < 10:
                    ReviewTagAssignment.objects.create(
                        result=result, tag=self.tag_high_quality, assigned_by=self.user
                    )

                # Add study type tags
                if i % 2 == 0:
                    ReviewTagAssignment.objects.create(
                        result=result, tag=self.tag_rct, assigned_by=self.user
                    )
                else:
                    ReviewTagAssignment.objects.create(
                        result=result, tag=self.tag_observational, assigned_by=self.user
                    )
    ...
    def test_synthesize_study_findings(self):
        ...
        self.assertIn(
            ["high", "moderate", "low", "very_low"], evidence["confidence_level"]
        )
    ...
    def test_logging_in_analysis(self):
        ...
        with self.assertLogs(
            "apps.reporting.services.study_analysis_service", level="INFO"
        ) as cm:
