...
class DeduplicationService(ServiceLoggerMixin):
    ...
    def calculate_similarity_score(self, text1: str, text2: str) -> float:
        ...
        if len(text1_words) > 0 and len(text2_words) > 0:
            word_overlap = len(text1_words & text2_words) / len(
                text1_words | text2_words
            )
    ...
    def extract_title_keywords(self, title: str) -> Set[str]:
        ...
        keywords = {
            word for word in words if word not in DeduplicationConstants.STOP_WORDS
        }
    ...
    def detect_duplicates(
        self,
        results: QuerySet,
        similarity_threshold: float = DeduplicationConstants.DEFAULT_SIMILARITY_THRESHOLD,
    ) -> List[Dict[str, Any]]:
        """
    ...
    def check_duplicate_methods(self, result1, result2) -> Dict[str, Any]:
        ...
        if result1.snippet and result2.snippet:
            snippet_similarity = self.calculate_similarity_score(
        ...
        return {"is_duplicate": False, "method": "none", "confidence": 0.0}
...
class DuplicateDetector:
    ...
    def _is_duplicate_match(self, similarity_info: Dict[str, Any]) -> bool:
        ...
        return (
            similarity_info["is_duplicate"]
            and similarity_info["confidence"] >= self.threshold
        )

    def _add_to_group(
        self, group: Dict[str, Any], result, similarity_info: Dict[str, Any]
    ) -> None:
