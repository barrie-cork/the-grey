...
class ProcessingAnalyticsService(ServiceLoggerMixin):
    ...
    def get_processing_statistics(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'pdf_percentage': round((pdf_count / total_results * ProcessingConstants.PERCENTAGE_MULTIPLIER), ProcessingConstants.DECIMAL_PLACES['percentage']) if total_results > 0 else 0,
    ...
    def get_results_for_review(self, session_id: str, limit: int = ProcessingConstants.DEFAULT_PRIORITIZATION_LIMIT) -> QuerySet:
        """
        Get results for manual review (simple ordering - no complex prioritization).
        ...
        Returns:
            QuerySet of ProcessedResult instances in simple order
        """
    ...
    def export_results_summary(self, session_id: str) -> Dict[str, Any]:
        ...
        for doc_type in stats['document_types'].keys():
            samples = results.filter(document_type=doc_type).order_by('-publication_year', '-is_pdf')[:ProcessingConstants.SAMPLE_RESULTS_LIMIT]
    ...
    def get_quality_distribution(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            'total_results': total,
            'pdf_count': results.filter(is_pdf=True).count(),
            'recent_count': results.filter(publication_year__gte=2020).count() if results.filter(publication_year__isnull=False).exists() else 0,
