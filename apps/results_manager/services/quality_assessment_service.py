...
class QualityAssessmentService(ServiceLoggerMixin):
    ...
    def assess_document_quality(self, result) -> dict:
        ...
        """
        quality = {"overall_score": 0.0, "quality_factors": [], "concerns": []}
        ...
        if result.publication_year:
            if result.publication_year >= QualityConstants.RECENT_PUBLICATION_THRESHOLD:
                ...
                result.publication_year >= QualityConstants.MODERATELY_RECENT_THRESHOLD
            ):
        ...
        if (
            result.snippet
            and len(result.snippet) >= QualityConstants.MIN_SNIPPET_LENGTH
        ):
