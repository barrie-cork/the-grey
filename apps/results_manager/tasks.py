...
def process_session_results_task(self, session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if self.request.retries < MAX_RETRIES:
            ...
            logger.info(f"Retrying processing for session {session_id} in {retry_countdown}s")
...
def create_processing_workflow(self, session_id: str, processing_session_id: str) -> Dict[str, Any]:
    """
...
@shared_task(bind=True, max_retries=MAX_RETRIES)
def process_batch_task(self, session_id: str, processing_session_id: str, raw_result_ids: List[str]) -> Dict[str, Any]:
    """
    ...
    """
    logger.info(f"Processing batch of {len(raw_result_ids)} results for session {session_id}")
...
def process_single_result(raw_result: RawSearchResult, session_id: str) -> Optional[ProcessedResult]:
    """
...
@shared_task(bind=True)
def run_deduplication_task(self, batch_results: List[Dict], session_id: str, processing_session_id: str) -> Dict[str, Any]:
    """
...
@shared_task(bind=True)
def finalize_processing_task(self, dedup_results: Dict, session_id: str, processing_session_id: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        processing_session.update_progress('quality_scoring', 50)
        ...
        logger.info(f"Processing completed for session {session_id}: {stats['processed_results']} results")
...
def cleanup_failed_processing(days_old: int = 7) -> Dict[str, Any]:
    ...
    logger.info(f"Cleaned up {count} failed processing sessions older than {days_old} days")
...
def retry_failed_processing_task(session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        RawSearchResult.objects.filter(
            execution__query__session_id=session_id
        ).update(is_processed=False, processing_error='')
