...
def document_type_badge(doc_type: str) -> str:
    ...
    return format_html('<span class="badge bg-secondary">{}</span>', display_name)
...
def document_indicators(result: ProcessedResult) -> str:
    ...
    if result.is_pdf:
        indicators.append(
            '<i class="bi bi-file-pdf text-danger" title="PDF document"></i>'
        )
    else:
    ...
    if result.duplicate_group:
        indicators.append(
            '<i class="bi bi-files text-secondary" title="Part of duplicate group"></i>'
        )
...
def publication_year_badge(year: int) -> str:
    ...
    return format_html('<span class="badge bg-secondary">{}</span>', year)
