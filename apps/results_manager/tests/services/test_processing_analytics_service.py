...
class TestProcessingAnalyticsService(TestCase):
    ...
    def test_logging_in_analytics(self):
        ...
        with self.assertLogs(
            "apps.results_manager.services.processing_analytics_service", level="INFO"
        ) as cm:
            self.service.calculate_processing_metrics(str(self.session.id))

        self.assertTrue(
            any("Calculating processing metrics" in msg for msg in cm.output)
        )
        self.assertTrue(
            any("Processing metrics calculated" in msg for msg in cm.output)
        )
