    ...
    def set_status_draft(self, request, queryset):
        for session in queryset:
            if session.can_transition_to("draft"):
                ...
                session.save()
        self.message_user(
            request, f"Updated {queryset.count()} sessions to Draft status"
        )
    ...
    def set_status_defining_search(self, request, queryset):
        for session in queryset:
            if session.can_transition_to("defining_search"):
                ...
                session.save()
        self.message_user(
            request, f"Updated {queryset.count()} sessions to Defining Search status"
        )
    ...
    def set_status_ready_to_execute(self, request, queryset):
        for session in queryset:
            if session.can_transition_to("ready_to_execute"):
                ...
                session.save()
        self.message_user(
            request, f"Updated {queryset.count()} sessions to Ready to Execute status"
        )
