...
def get_session_statistics(user: User) -> Dict[str, Any]:
    ...
    return {
        ...
        'overall_inclusion_rate': round((total_included / total_reviewed * 100), 1) if total_reviewed > 0 else 0,
        'active_sessions': sessions.exclude(status__in=['completed', 'archived']).count(),
        'completed_sessions': sessions.filter(status='completed').count(),
    }


def validate_status_transition(session: SearchSession, new_status: str) -> Tuple[bool, Optional[str]]:
    """
...
def transition_session_status(session: SearchSession, new_status: str, user: Optional[User] = None, 
...
def get_recent_activities(session: SearchSession, limit: int = 10) -> QuerySet:
    ...
    """
    return (SessionActivity.objects
...
def calculate_workflow_progress(session: SearchSession) -> Dict[str, Any]:
    ...
    """
    status_order = [status[0] for status in SearchSession.STATUS_CHOICES if status[0] != 'archived']
    current_index = status_order.index(session.status) if session.status in status_order else 0
    total_steps = len(status_order)
    
    return {
        'current_step': current_index + 1,
        'total_steps': total_steps,
        'percentage': round((current_index / (total_steps - 1)) * 100, 1) if total_steps > 1 else 0,
        'status_display': session.get_status_display(),
        'next_statuses': [dict(SearchSession.STATUS_CHOICES)[status] for status in session.get_allowed_transitions()],
...
def get_dashboard_summary(user: User) -> Dict[str, Any]:
    ...
    active_sessions = sessions.exclude(status__in=['completed', 'archived', 'draft']).count()
    completed_sessions = sessions.filter(status='completed').count()
    ...
    # Calculate totals
    total_results = sessions.aggregate(
        total=Count('total_results'))['total'] or 0
    ...
    return {
        ...
        'requires_attention': sessions.filter(
            Q(status='ready_to_execute') | 
            Q(status='ready_for_review')
        ).count(),
    }


def bulk_update_session_tags(sessions: List[SearchSession], tags_to_add: List[str], 
                           tags_to_remove: List[str] = None) -> int:
    """
...
def get_sessions_needing_attention(user: User) -> QuerySet:
    ...
    return (SearchSession.objects
            .filter(owner=user, status__in=attention_statuses)
            .order_by('updated_at'))