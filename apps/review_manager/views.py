    ...
    def get_queryset(self) -> QuerySet[SearchSession]:
        ...
        if status_filter and status_filter != "all":
            if status_filter == "active":
                queryset = queryset.exclude(status__in=["completed", "archived"])
            else:
...
class SessionCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form) -> HttpResponse:
        ...
        messages.success(
            self.request, f'Review session "{self.object.title}" created successfully!'
        )
        return response
...
class SessionDetailView(
    LoginRequiredMixin, UserOwnerMixin, SessionNavigationMixin, DetailView
):
    """
...
class SessionDeleteView(LoginRequiredMixin, UserOwnerMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(request, f'Session "{session_title}" has been deleted.')
        return response
...
class ArchiveSessionView(LoginRequiredMixin, UserOwnerMixin, View):
    ...
    def post(self, request, session_id):
        ...
        if not session.can_transition_to("archived"):
            messages.error(
                request, "This session cannot be archived in its current state."
            )
...
class SessionNavigateView(
    LoginRequiredMixin, UserOwnerMixin, SessionNavigationMixin, View
):
