...
class TaggingManagementService(ServiceLoggerMixin):
    ...
    def get_tag_usage_statistics(self, session_id: str) -> Dict[str, Any]:
        ...
        # Tag counts
        tag_counts = assignments.values('tag__name').annotate(
            count=Count('tag__name')
        ).order_by('-count')
        ...
        tag_type_distribution = {item['tag__tag_type']: item['count'] for item in type_counts}
    ...
    def bulk_tag_results(self, result_ids: List[str], tag_ids: List[str], user: User, notes: str = "") -> int:
        """
    ...
    def bulk_remove_tags(self, result_ids: List[str], tag_ids: List[str], user: User) -> int:
        """
    ...
    def get_tag_consistency_analysis(self, session_id: str) -> Dict[str, Any]:
        ...
        agreement_rate = self._calculate_agreement_rate(result_reviews, consistent_reviews)
        
        return {
            'session_id': session_id,
            'total_results_reviewed': len(result_reviews),
            'multi_reviewed_results': len([r for r in result_reviews.values() if len(r) > 1]),
    ...
    def _analyze_review_conflicts(self, result_reviews: Dict[str, List]) -> tuple:
        ...
        for result_id, reviews in result_reviews.items():
            if len(reviews) > 1:
                decision_tags = [r.tag.name for r in reviews if r.tag.tag_type == 'decision']
    ...
    def _calculate_agreement_rate(self, result_reviews: Dict[str, List], consistent_reviews: int) -> float:
    ...
    def create_custom_tag(self, name: str, tag_type: str, description: str, user: User) -> Dict[str, Any]:
        """
