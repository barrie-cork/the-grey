...
class TestReviewAnalyticsService(TestCase):
    ...
    def setUp(self):
        ...
        self.tag_include = ReviewTag.objects.create(
            name="Include", description="Include in final review"
        )
        self.tag_exclude = ReviewTag.objects.create(
            name="Exclude", description="Exclude from final review"
        )
        self.tag_maybe = ReviewTag.objects.create(
            name="Maybe", description="Requires further review"
        )
    ...
    def test_export_review_data(self):
        ...
        export_data = self.service.export_review_data(
            str(self.session.id), format_type="csv"
        )
    ...
    def test_validate_review_completeness(self):
        ...
        self.assertTrue(
            any("not been reviewed" in issue for issue in validation["issues"])
        )
    ...
    def test_export_with_multiple_tags(self):
        ...
        multi_tag_result = next(
            r for r in export_data["results"] if str(r["id"]) == str(self.results[0].id)
        )
    ...
    def test_calculate_exclusion_distribution(self):
        ...
        for i in range(8, 15):
            assignment = ReviewTagAssignment.objects.get(
                result=self.results[i], tag=self.tag_exclude
            )
            reasons = [
                "Out of scope",
                "Poor quality",
        ...
        excluded_results = [r for r in export_data["results"] if "Exclude" in r["tags"]]
    ...
    def test_reviewer_agreement_analysis(self):
        ...
        ReviewTagAssignment.objects.create(
            result=result, tag=self.tag_include, assigned_by=self.user2
        )
    ...
    def test_export_format_variations(self):
        ...
        for format_type in formats:
            export_data = self.service.export_review_data(
                str(self.session.id), format_type=format_type
            )
    ...
    def test_notes_aggregation(self):
        ...
        # Find the result
        noted_result = next(
            r for r in export_data["results"] if str(r["id"]) == str(result.id)
        )
    ...
    def test_logging_in_analytics(self):
        ...
        with self.assertLogs(
            "apps.review_results.services.review_analytics_service", level="INFO"
        ) as cm:
