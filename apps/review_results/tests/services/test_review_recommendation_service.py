...
from apps.review_results.services.review_recommendation_service import (
...
class TestReviewRecommendationService(TestCase):
    ...
    def _create_historical_data(self):
        ...
        for i in range(50):
            ...
            ReviewTagAssignment.objects.create(
                result=result, tag=tag, assigned_by=self.user1
            )
    ...
    def test_generate_recommendations(self):
        ...
        recommendations = self.service.generate_recommendations(
            str(self.session.id), user_id=self.user1.id
        )
    ...
    def test_prioritize_review_queue(self):
        ...
        unreviewed_results = ProcessedResult.objects.filter(
            session=self.session, reviewtagassignment__isnull=True
        )

        prioritized = self.service.prioritize_review_queue(
            list(unreviewed_results), str(self.session.id)
        )
    ...
    def test_analyze_reviewer_patterns(self):
        ...
        patterns = self.service.analyze_reviewer_patterns(
            self.user1.id, str(self.historical_session.id)
        )
    ...
    def test_predict_review_decision(self):
        ...
        prediction = self.service.predict_review_decision(result, self.user1.id)
    ...
    def test_detect_review_inconsistencies(self):
        ...
        ReviewTagAssignment.objects.create(
            result=similar_result1, tag=self.tag_include, assigned_by=self.user1
        )

        ReviewTagAssignment.objects.create(
            result=similar_result2, tag=self.tag_exclude, assigned_by=self.user1
        )
    ...
    def test_collaborative_filtering_recommendations(self):
        ...
        for i in range(5, 10):
            ...
            ReviewTagAssignment.objects.create(
                result=result, tag=tag, assigned_by=self.user2
            )

        collab_recs = self.service.generate_collaborative_recommendations(
            str(self.session.id), self.user1.id
        )
    ...
    def test_adaptive_learning(self):
        ...
        actual_tag = (
            self.tag_exclude
            if initial_rec["predicted_tag"] == "Include"
            else self.tag_include
        )

        ReviewTagAssignment.objects.create(
            result=result, tag=actual_tag, assigned_by=self.user1
        )

        # Update model with feedback
        self.service.update_recommendation_model(str(self.session.id), self.user1.id)
    ...
    def test_handle_cold_start(self):
        ...
        recommendations = self.service.generate_recommendations(
            str(new_session.id), self.user1.id
        )

        self.assertIsInstance(recommendations, list)
        # Should use rule-based approach for cold start
        self.assertTrue(
            any("rule-based" in r.get("reasoning", "") for r in recommendations)
        )

    def test_logging_in_recommendations(self):
        """Test that recommendation operations are properly logged."""
        with self.assertLogs(
            "apps.review_results.services.review_recommendation_service", level="INFO"
        ) as cm:
