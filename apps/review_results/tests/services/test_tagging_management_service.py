...
from apps.review_results.services.tagging_management_service import TaggingManagementService
...
class TestTaggingManagementService(TestCase):
    ...
    def setUp(self):
        ...
        self.tag_exclude = ReviewTag.objects.create(
            name='Exclude',
            description='Exclude from review',
            color='#dc3545'
        )
    ...
    def test_get_tag_usage_statistics(self):
        ...
        self.assertTrue(any('Include + High Quality' in combo for combo in combinations))
    ...
    def test_tag_conflict_detection(self):
        ...
        result_conflict = next(
            ...
            if str(c['result_id']) == str(result.id)
        )
    ...
    def test_tag_usage_trends(self):
        ...
        trends = self.service.analyze_tag_usage_trends(
            str(self.session.id),
            days=7
        )
    ...
    def test_tag_merge_operations(self):
        ...
        ReviewTagAssignment.objects.create(
            result=self.results[22],
            tag=tag1,
            assigned_by=self.user1
        )
        
        ReviewTagAssignment.objects.create(
            result=self.results[23],
            tag=tag2,
            assigned_by=self.user1
        )
    ...
    def test_logging_in_tagging(self):
        ...
        with self.assertLogs('apps.review_results.services.tagging_management_service', level='INFO') as cm:
            self.service.get_tag_usage_statistics(str(self.session.id))
        
        self.assertTrue(any('Calculating tag usage statistics' in msg for msg in cm.output))
