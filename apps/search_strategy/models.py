...
class SearchStrategy(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="search_strategies"
    )
    ...
    def generate_queries(self):
        ...
        if include_general:
            ...
            queries.append({"query": general_query, "domain": None, "type": "general"})
...
class QueryTemplate(models.Model):
    ...
    category = models.CharField(
        max_length=100, blank=True, help_text="Category for organizing templates"
    )
    ...
    is_public = models.BooleanField(
        default=False, help_text="Whether this template is available to all users"
    )
    ...
    default_keywords = models.JSONField(
        default=list, blank=True, help_text="Default keywords to include"
    )
    default_exclusions = models.JSONField(
        default=list, blank=True, help_text="Default keywords to exclude"
    )
    default_engines = models.JSONField(
        default=list, blank=True, help_text="Default search engines"
    )
    ...
    def create_query_from_template(
        self, session: SearchSession, **kwargs: Any
    ) -> SearchStrategy:
        """
