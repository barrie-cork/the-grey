...
def pic_badge(component: str, component_type: str) -> str:
    ...
    if not component or not component.strip():
        return format_html(
            '<span class="badge bg-light text-muted">Not specified</span>'
        )

    colors = {"population": "primary", "interest": "success", "context": "info"}
...
def keyword_list(keywords: List[str], max_display: int = 5) -> str:
    ...
    for keyword in displayed:
        html_parts.append(
            format_html(
                '<span class="badge bg-light text-dark me-1">{}</span>', keyword
            )
        )

    if remaining > 0:
        html_parts.append(
            format_html('<span class="badge bg-secondary">+{} more</span>', remaining)
        )
...
def search_engines_badges(engines: List[str]) -> str:
    ...
    for engine in engines:
        ...
        badges.append(
            format_html(
                '<span class="badge bg-{} me-1">{}</span>', color, engine.title()
            )
        )
...
def query_summary_card(query: SearchQuery):
    ...
    return {
        ...
        "has_filters": bool(
            query.date_from or query.date_to or query.languages or query.document_types
        ),
...
def date_range_display(query: SearchQuery) -> str:
    ...
    date_from_str = (
        query.date_from.strftime("%Y-%m-%d") if query.date_from else "Beginning"
    )
...
def template_usage_badge(template: QueryTemplate) -> str:
    ...
    if template.usage_count == 0:
        ...
        return format_html(
            '<span class="badge bg-info">{} uses</span>', template.usage_count
        )
    elif template.usage_count < 20:
        return format_html(
            '<span class="badge bg-success">{} uses</span>', template.usage_count
        )
    else:
        return format_html(
            '<span class="badge bg-warning">{} uses</span>', template.usage_count
        )
...
def execution_history_icon(query: SearchQuery) -> str:
    ...
    last_executed = (
        query.last_executed.strftime("%Y-%m-%d %H:%M")
