...
def validate_pic_framework(population: str, interest: str, context: str) -> Tuple[bool, List[str]]:
    """
...
def generate_query_variations(base_query: SearchQuery, variation_count: int = 3) -> List[Dict[str, Any]]:
    """
...
def _broaden_term(term: str) -> str:
    ...
    broad_term = re.sub(r'\b(specific|particular|exact|precise|detailed)\s+', '', term, flags=re.IGNORECASE)
...
def _add_synonyms(term: str) -> str:
    ...
    for word, replacement in synonyms.items():
        if word.lower() in term.lower():
            modified_term = re.sub(r'\b' + re.escape(word) + r'\b', replacement, modified_term, flags=re.IGNORECASE)
...
def optimize_query_string(query_string: str) -> Dict[str, Any]:
    ...
    analysis = {
        ...
        'has_boolean_operators': any(op in query_string.upper() for op in ['AND', 'OR', 'NOT']),
    ...
    if analysis['word_count'] > 15:
        analysis['suggestions'].append('Consider shortening the query - very long queries may be too specific')
    
    if analysis['word_count'] < 3:
        analysis['suggestions'].append('Consider adding more descriptive terms to improve precision')
    
    if not analysis['has_quotes'] and analysis['word_count'] > 5:
        analysis['suggestions'].append('Consider using quotes around key phrases for exact matches')
    
    if not analysis['has_boolean_operators'] and analysis['word_count'] > 8:
        analysis['suggestions'].append('Consider using AND/OR operators to structure complex queries')
    
    # Check for common issues
    if re.search(r'\b(a|an|the|is|are|was|were|be|been|being)\b', query_string, re.IGNORECASE):
        analysis['suggestions'].append('Consider removing common stop words (a, an, the, is, are, etc.)')
    
    return analysis


def get_template_suggestions(population: str, interest: str, context: str, user: User) -> QuerySet:
    """
...
def calculate_query_complexity(query: SearchQuery) -> Dict[str, Any]:
    ...
    if query.include_keywords:
        ...
        complexity['factors'].append(f"{len(query.include_keywords)} additional keywords")
    
    if query.exclude_keywords:
        complexity['score'] += len(query.exclude_keywords) * 1.5
        complexity['factors'].append(f"{len(query.exclude_keywords)} exclusion keywords")
    ...
    if query.document_types:
        ...
        complexity['factors'].append(f"{len(query.document_types)} document type filters")
...
def get_session_query_statistics(session_id: str) -> Dict[str, Any]:
    ...
    stats = {
        ...
        'avg_max_results': 0,
        'total_max_results': 0,
        'complexity_distribution': {'Simple': 0, 'Moderate': 0, 'Complex': 0, 'Very Complex': 0},
...
def export_queries_to_dict(session_id: str) -> List[Dict[str, Any]]:
    ...
    """
    queries = SearchQuery.objects.filter(session_id=session_id).order_by('order', 'created_at')
