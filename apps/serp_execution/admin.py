...
class SearchExecutionAdmin(admin.ModelAdmin):
    ...
    def query_link(self, obj):
        ...
        if obj.query:
            url = reverse('admin:search_strategy_searchquery_change', args=[obj.query.id])
            return format_html('<a href="{}">{}</a>', url, obj.query.query_string[:50] + '...')
        return '-'
    ...
    def retry_failed_executions(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Started retry for {count} failed executions."
        )
...
class RawSearchResultAdmin(admin.ModelAdmin):
    ...
    def detect_academic_sources(self, request, queryset):
        ...
        count = 0
        academic_domains = [
            '.edu', '.ac.uk', 'scholar.google', 'arxiv.org', 
