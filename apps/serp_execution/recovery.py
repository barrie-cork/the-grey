...
class ExponentialBackoffStrategy(RecoveryStrategy):
    ...
    def get_delay(self) -> float:
        ...
        delay = min(self.BASE_DELAY * (2 ** (self.attempt_number - 1)), self.MAX_DELAY)
        # Add jitter (±20%)
        import random
...
class QuerySimplificationStrategy(RecoveryStrategy):
    ...
    def modify_request(self) -> Dict[str, Any]:
        ...
        if self.attempt_number == 1:
            ...
            query = re.sub(
                r"[-+]?\w+:", "", query
            )  # Remove operators like -word or filetype:
...
class QuotaExhaustedStrategy(RecoveryStrategy):
    ...
    def get_notification_message(self) -> Optional[str]:
        return (
            "API quota exhausted. Please check your Serper account or contact support."
        )
...
class ErrorRecoveryManager:
    ...
    def _track_recovery_attempt(
        self, execution_id: str, error: Exception, strategy: RecoveryStrategy
    ):
    ...
    def get_manual_intervention_required(
        ...
        network_errors = [
            h for h in history if h["error_type"] in ["ConnectionError", "TimeoutError"]
        ]
            ...
            interventions.append(
                {
                    ...
                    "suggestions": self._get_query_simplification_suggestions(
                        execution
                    ),
                }
            )
    ...
    def record_successful_recovery(self, execution_id: str):
        ...
        if execution_id in self.recovery_history:
            logger.info(
                f"Successful recovery for execution {execution_id} after {len(self.recovery_history[execution_id])} attempts"
            )
