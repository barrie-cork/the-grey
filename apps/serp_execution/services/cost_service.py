...
class CostService(ServiceLoggerMixin):
    ...
    def calculate_api_cost(
        self, credits_used: int, rate_per_credit: Decimal = Decimal("0.001")
    ) -> Decimal:
        """
    ...
    def get_actual_session_cost(self, session_id: str) -> Dict[str, Any]:
        ...
        executions = SearchExecution.objects.filter(
            query__strategy__session_id=session_id, status="completed"
        )

        total_credits = (
            executions.aggregate(total=Sum("api_credits_used"))["total"] or 0
        )
        ...
        return {
            ...
            "average_cost_per_execution": (
                total_cost / executions.count() if executions.count() > 0 else 0
            ),
        }

    def check_budget_status(
        self, session_id: str, budget_limit: Decimal
    ) -> Dict[str, Any]:
        """
    ...
    def calculate_session_cost_estimate(self, session_id: str) -> Dict[str, Any]:
        ...
        for query in queries_data:
            ...
            query_detail = {
                "id": query.get("id"),
                "query_string": (
                    query.get("query_string", "")[:50] + "..."
                    if len(query.get("query_string", "")) > 50
                    else query.get("query_string", "")
                ),
                "engines": engines,
    ...
    def _get_average_cost_per_query(self) -> Decimal:
        ...
        recent_executions = SearchExecution.objects.filter(
            completed_at__gte=thirty_days_ago, status="completed"
        )
        ...
        total_credits = (
            recent_executions.aggregate(total=Sum("api_credits_used"))["total"] or 0
        )
