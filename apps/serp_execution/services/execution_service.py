...
class ExecutionService(ServiceLoggerMixin):
    ...
    def estimate_execution_time(
        self, query_complexity: int, search_engines: List[str]
    ) -> int:
        """
    ...
    def validate_execution_readiness(self, session_id: str) -> Dict[str, Any]:
        ...
        validation_result = {"is_ready": True, "issues": [], "warnings": []}
        ...
        for query in queries_data:
            if not query.get("query_string"):
                validation_result["is_ready"] = False
                validation_result["issues"].append(
                    f"Query '{query.get('title')}' has no query string"
                )

            if not query.get("search_engines"):
                validation_result["warnings"].append(
                    f"Query '{query.get('title')}' has no search engines specified"
                )
    ...
    def calculate_search_coverage(self, session_id: str) -> Dict[str, Any]:
        ...
        executed_engines = set(
            executions.filter(status="completed").values_list(
                "search_engine", flat=True
            )
        )
        ...
        return {
            ...
            "query_coverage_percentage": (
                round((executed_queries / total_queries) * 100, 1)
                if total_queries > 0
                else 0
            ),
            ...
            "engine_coverage_percentage": (
                round((len(executed_engines) / len(planned_engines)) * 100, 1)
                if planned_engines
                else 0
            ),
        }
