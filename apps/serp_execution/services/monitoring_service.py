...
class MonitoringService(ServiceLoggerMixin):
    ...
    def get_execution_statistics(self, session_id: str) -> Dict[str, Any]:
        ...
        success_rate = self._calculate_success_rate(counts['total_executions'], counts['successful_executions'])
        ...
        return {
            **counts,
            ...
            'engines_used': list(executions.values_list('search_engine', flat=True).distinct()),
            'last_execution_date': self._get_last_execution_date(executions)
        }
    
    def get_failed_executions_with_analysis(self, session_id: str) -> List[Dict[str, Any]]:
        """
        ...
        for execution in failed_executions:
            analysis = {
                ...
                'query_title': execution.query.query_string[:50] + '...' if len(execution.query.query_string) > 50 else execution.query.query_string,
    ...
    def optimize_retry_strategy(self, execution) -> Dict[str, Any]:
        ...
        if failure_category == 'rate_limit':
            strategy.update({
                'should_retry': True,
                'delay_seconds': min(300 * (execution.retry_count + 1), 3600),  # Exponential backoff, max 1 hour
                'priority': 'medium',
                'modifications': ['Use exponential backoff', 'Consider alternative engine']
            })
            ...
            strategy.update({
                ...
                'delay_seconds': 120,
                'priority': 'high',
                'modifications': ['Check network connectivity', 'Use different endpoint']
            })
            ...
            strategy.update({
                'should_retry': execution.retry_count < 2,  # More conservative for unknown errors
                'delay_seconds': 300,
                'priority': 'low',
    ...
    def get_engine_performance_comparison(self, session_id: str) -> Dict[str, Dict[str, Any]]:
        """
        ...
        for execution in executions:
            ...
            if execution.status == 'completed':
                ...
                if execution.duration_seconds:
                    ...
                    engines[engine]['average_time'] = ((current_avg * (count - 1)) + execution.duration_seconds) / count
    ...
    def calculate_search_coverage(self, session_id: str) -> Dict[str, Any]:
        ...
        for result in raw_results:
            ...
            content_info = content_analyzer.detect_content_type(result.link, result.title, result.snippet)
        ...
        domain_distribution = dict(sorted(domain_distribution.items(), key=lambda x: x[1], reverse=True)[:10])
        
        academic_percentage = (academic_count / total_results * 100) if total_results > 0 else 0
        ...
        for execution in executions:
            ...
            if execution.status == 'failed' and execution.updated_at:
                timeline.append({
                    ...
                    'query': execution.query.query_string[:50] + '...',
                    'engine': execution.search_engine,
                    'error': execution.error_message[:100] + '...' if len(execution.error_message) > 100 else execution.error_message
                })
    ...
    def format_execution_status(self, status: str) -> Dict[str, str]:
        ...
        return status_formats.get(status, {'icon': 'question-circle', 'class': 'text-muted'})
