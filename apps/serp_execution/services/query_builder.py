...
class QueryBuilder:
    ...
    ACADEMIC_DOMAINS = [
        'edu', 'ac.uk', 'edu.au', 'edu.ca', 'ac.in', 'edu.cn',
        'univ', 'university', 'college', 'institute', 'academia.edu',
        'researchgate.net', 'arxiv.org', 'ssrn.com', 'nih.gov',
        'gov', 'org', 'ngo'
    ]
    ...
    def build_query(
        ...
        try:
            ...
            if exact_phrases:
                ...
                base_query = f"{base_query} {' '.join(phrase_parts)}" if base_query else ' '.join(phrase_parts)
            ...
            if include_keywords:
                ...
                if keyword_parts:
                    ...
                    base_query = f"{base_query} ({keywords_string})" if base_query else keywords_string
            
            # Add exclusions
            if exclude_keywords:
                exclusions = ' '.join([f'-"{word}"' if ' ' in word else f'-{word}' 
    ...
    def _process_component(self, component: str, component_type: str) -> str:
        ...
        if ' ' in component and not any(op in component.upper() for op in ['AND', 'OR', 'NOT']):
    ...
    def _remove_stop_words(self, query: str) -> str:
        ...
        for word in words:
            if (word.upper() in ['AND', 'OR', 'NOT'] or 
                word.startswith('-') or 
                word.startswith('site:') or 
                word.startswith('filetype:') or
                word.startswith('__PHRASE_') or
    ...
    def extract_key_terms(self, query: str) -> List[str]:
        ...
        terms = [term.strip() for term in clean_query.split() 
