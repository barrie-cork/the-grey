...
class ResultProcessor:
    ...
    def process_search_results(
        self, execution_id: str, raw_results: List[Dict[str, Any]], batch_size: int = 50
        ...
        for i in range(0, len(raw_results), batch_size):
            ...
            try:
                with transaction.atomic():
                    batch_processed, batch_duplicates, batch_errors = (
                        self._process_batch(execution, batch, starting_position=i + 1)
                    )
        ...
        logger.info(
            f"Processed {processed_count} results, found {duplicate_count} duplicates"
        )
        ...
        for file_type, indicators in self.FILE_TYPE_INDICATORS.items():
            for indicator in indicators:
                if (
                    ...
                    or indicator in snippet
                    or (mime_type and indicator in mime_type)
                ):
                    return file_type
    ...
    def _is_academic_source(self, url: str, result: Dict[str, Any]) -> bool:
        ...
        academic_keyword_count = sum(
            1 for keyword in academic_keywords if keyword in text_to_check
        )
    ...
    def prepare_for_processing_pipeline(self, raw_result_id: str) -> Dict[str, Any]:
        """
