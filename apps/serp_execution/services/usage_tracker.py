...
from apps.serp_execution.models import SearchExecution, ExecutionMetrics
...
class UsageTracker:
    ...
    def check_budget_status(self) -> Dict[str, Any]:
        ...
        return {
            'daily': {
                ...
                'remaining': float(Decimal(str(self.daily_budget)) - daily_usage['total_cost']),
                'percentage': float(daily_usage['total_cost'] / Decimal(str(self.daily_budget))) if self.daily_budget > 0 else 0,
                'queries': daily_usage['total_queries']
            },
            'monthly': {
                'used': float(monthly_usage['total_cost']),
                'budget': float(self.monthly_budget),
                'remaining': float(Decimal(str(self.monthly_budget)) - monthly_usage['total_cost']),
                'percentage': float(monthly_usage['total_cost'] / Decimal(str(self.monthly_budget))) if self.monthly_budget > 0 else 0,
    ...
    def can_execute_queries(self, num_queries: int) -> Tuple[bool, Optional[str]]:
        ...
        if budget_status['daily']['remaining'] < estimated_cost:
            return False, f"Insufficient daily budget. Need ${estimated_cost:.3f}, have ${budget_status['daily']['remaining']:.3f}"
        
        # Check monthly budget
        if budget_status['monthly']['remaining'] < estimated_cost:
            return False, f"Insufficient monthly budget. Need ${estimated_cost:.3f}, have ${budget_status['monthly']['remaining']:.3f}"
        
        # Check if we're at critical threshold
        new_daily_percentage = (Decimal(str(budget_status['daily']['used'])) + estimated_cost) / Decimal(str(self.daily_budget))
        if float(new_daily_percentage) >= self.BUDGET_CRITICAL_THRESHOLD:
            return False, f"Execution would exceed {self.BUDGET_CRITICAL_THRESHOLD:.0%} of daily budget"
    ...
    def _update_session_metrics(self, execution: SearchExecution) -> None:
        ...
        """
        try:
            session = execution.query.session
    ...
    def _generate_usage_warnings(self, budget_status: Dict[str, Any]) -> List[str]:
        ...
        if daily_percentage >= self.BUDGET_CRITICAL_THRESHOLD:
            warnings.append(
                f"CRITICAL: Daily budget usage at {daily_percentage:.1%}"
            )
        elif daily_percentage >= self.BUDGET_WARNING_THRESHOLD:
            warnings.append(
                f"WARNING: Daily budget usage at {daily_percentage:.1%}"
            )
